 ...
 * ・1行100文字幅
 * ・セミコロンは全ての場所につける。
 * ・ダブルクオートを"処理向け文字列"とし、シングルクオートを'利用者向け文字列'としていたが、v2.58
 *   現在一貫性に漏れがある
 * ・文字列定数は直接書く。変数を媒介しない。
 */
...
(function() {
  ...
  function modYamanuchangForSportsChan() {
    ...
    window.yamanu.runOnBodyAvailable.append(window.yamanu.insertCSS = function insertCSS() {

      var style = document.createElement('STYLE');
      style.type = "text/css";
      style.id = "yamanuchanCSS";

      style.innerHTML +=
        ".hidden_post .body," +
        ".hidden_post .files," +
        ".hidden_post .intro label" +
        "{ display: none; }";
    ...
    window.yamanu.hiddenPosts = JSON.parse(localStorage.hiddenposts || "{}");

    window.yamanu.storeHiddenPostsData = function storeHiddenPostsData() {
      localStorage.hiddenposts = JSON.stringify(window.yamanu.hiddenPosts);
    };

    window.yamanu.showHidePost = function showHidePost(unused, post) {
      var board = window.yamanu.getPostBoard(post);
      var number = window.yamanu.getPostNumber(post);

      if (board && number && window.yamanu.hiddenPosts[board] && window.yamanu.hiddenPosts[board][number]) {

        post.className += " hidden_post";
      };
    };

    window.yamanu.runOnBodyAvailable.append(window.yamanu.showHidePostAll
      ...
      for (var i = postList.length - 1; 0 <= i ; --i) {
        ...
        window.yamanu.showHidePost(null, postList[i]);
      };
    });

    window.yamanu.runOnBodyAvailable.append(window.yamanu.setFuncShowHidePost
        = function setFuncShowHidePost() {

      $(document).on("new_post", window.yamanu.showHidePost);
    });


    window.yamanu.hidePost = function hidePost() {
      var post = window.yamanu.getContextTargetPost();
      post.className += " hidden_post";

      var board = window.yamanu.getPostBoard(post);
      var number = window.yamanu.getPostNumber(post);

      if (null === board || null === number) {
        return;
      };

      window.yamanu.hiddenPosts[board] = window.yamanu.hiddenPosts[board] || {};
      window.yamanu.hiddenPosts[board][number] = 1;

      window.yamanu.storeHiddenPostsData();
    };

    window.yamanu.showPost = function showPost() {
      var post = window.yamanu.getContextTargetPost();
      post.className = post.className.replace(/\b\s*hidden_post\b/g, "");

      var board = window.yamanu.getPostBoard(post);
      var number = window.yamanu.getPostNumber(post);

      if (null === board || null === number || undefined === window.yamanu.hiddenPosts[board]) {
        return;
      };

      delete window.yamanu.hiddenPosts[board][number];

      window.yamanu.storeHiddenPostsData();
    };


    window.yamanu.getContextTargetPost = function getContextTargetPost() {
      /* vichan/js/quick-post-controls.js によって挿入される post-menu が target としている post div を返す */

      var list = document.getElementsByClassName("post-btn-open");
      if (0 === list.length) {
        return null;
      };
      return list[0].parentElement.parentElement;
    };

    window.yamanu.opRegexp = /\bop\b/;
    window.yamanu.addPostMenu = function addPostMenu(postMenu) {

      var post = window.yamanu.getContextTargetPost();

      if (null === post || window.yamanu.opRegexp.test(post.className)) {
        return;
      };



      var ul = postMenu.firstChild;
      var li = document.createElement("LI");

      li.className = "post-item";

      if (0 <= post.className.indexOf("hidden_post")) {
        li.id = "show_post_menu";
        li.textContent = "Show post";
        li.addEventListener("click", window.yamanu.showPost);
      } else {
        li.id = "hide_post_menu";
        li.textContent = "Hide post";
        li.addEventListener("click", window.yamanu.hidePost);
      };

      ul.appendChild(li);
    };

    window.yamanu.postMenuHook = function postMenuHook(postMenu) {

      window.yamanu.addPostMenu(postMenu);
    };

    window.yamanu.postMenuMO = undefined;
    window.yamanu.runOnBodyAvailable.append(window.yamanu.startPostMenuMO = function postMenuMO() {

      window.yamanu.postMenuMO = new MutationObserver(window.yamanu.postMenuHook_filter1);
      window.yamanu.postMenuMO.observe(document.body, {childList: true});
    });

    window.yamanu.postMenuHook_filter1 = function postMenuHook_filter1(mutationRecords) {

      for (var mrIdx = 0, mrLen = mutationRecords.length; mrIdx < mrLen ; ++mrIdx) {
        var mr = mutationRecords[mrIdx];
        for (var anIdx = 0, anLen = mr.addedNodes.length; anIdx < anLen ; ++anIdx) {

          var node = mr.addedNodes[anIdx];
          if (0 <= node.className.indexOf("post-menu")) {

            window.yamanu.postMenuHook(node);
          };
        };
      };
    };

    window.yamanu.getPostNoElement = function getPostNoElement(post) {
      var postNoList = post.getElementsByClassName("post_no");
      if (0 === postNoList.length) {
        return null;
      };
      return postNoList[0];
    };

    window.yamanu.postBoardRegexp = /[^\/]*\/\/[^\/]*\/([^\/]*)\/.*/;
    window.yamanu.getPostBoard = function getPostBoard(post) {
      var postNo = window.yamanu.getPostNoElement(post);
      if (null === postNo) {
        return null;
      };

      return postNo.href.replace(window.yamanu.postBoardRegexp, "$1");
    };

    window.yamanu.threadNumberRegexp = /.*\/\/.*\/[^\/]*\/res\/([0-9]*)\.html.*/;
    window.yamanu.postNumberRegexp = /.*#([0-9]*).*/;
    window.yamanu.getPostNumber = function getPostNumber(post) {
      var postNo = window.yamanu.getPostNoElement(post);
      if (null === postNo) {
        return null;
      };

      return postNo.href.replace(window.yamanu.postNumberRegexp, "$1");
    };
